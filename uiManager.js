...
export class UIManager {
  ...
  attachToggleEventListeners() {
    ...
    document.querySelectorAll('textarea').forEach(textarea => {
      ...
      let lastCursorPosition = textarea.selectionStart;

      const updateScroll = () => {
        if (textarea.scrollHeight !== lastScrollHeight) {
          const cursorPosition = textarea.selectionStart;
          const scrollPercentage = lastScrollTop / (lastScrollHeight - textarea.clientHeight);
      ...
      textarea.addEventListener('input', (e) => {
        ...
        requestAnimationFrame(() => {
          updateScroll();
          if (currentCursorPosition === textarea.value.length) {
      ...
      textarea.addEventListener('focus', () => {
        ...
        if (textareaRect.bottom > viewportHeight - keyboardHeight) {
          const editorContent = textarea.closest('.editor-content');
          const scrollOffset = textareaRect.top - 80;
